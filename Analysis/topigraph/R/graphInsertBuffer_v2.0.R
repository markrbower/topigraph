graphInsertBuffer_v2.0 <- function( parameters, cw, cc, ed, gf, blackout, dib_=NULL, state=NULL, conn=NULL, P_table=NULL ) {
  #' @export
  #' 
...
#    rm(ed)
  ...
  appendToGraph <- function() { # Add new events and links from the lists to the graph
    ...
    holdOnto_events <- events[which( event_times > t_graphWindow )]
    events_ <- events[ which( event_times <= t_graphWindow & event_times >= t_storeWindow )]

    # Which links to keep out of the graph?
    # If the times are stored [larger,smaller], then the graph holds links
    # where the first and second time will still be in the graph.
    firstTimes <- unname( sapply( names(links), function(x) as.numeric( unlist( str_split(x,',' ) ) )[1] ) )
    secondTimes <- unname( sapply( names(links), function(x) as.numeric( unlist( str_split(x,',' ) ) )[2] ) )
    holdOnto_links <- links[which(firstTimes > t_graphWindow )]
    links_ <- links[which(firstTimes <= t_graphWindow & secondTimes >= t_storeWindow )]
  ...
  processSubgraph <- function( Nlimit, append=TRUE ) {
    ...
    if ( !graphChanged ) {
      print( 'Graph not changed. Returning.')
      return(0)
    }
...
#    print( "Leaving update")
  ...
  persistGraph <- function( state ) { # Send the entire, current graph to the database.
    ...
    state <- processSubgraph( max_event_time+1, FALSE ) # 'append' is FALSE, beacuse 'makeGraph' appended, already.
    print( "Leaving persist")
    return( state )
  }
